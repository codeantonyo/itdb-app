...
/** POST /api/itdbone/actions — collect accrued ITDBONE daily yield. */
export async function POST(req: Request) {
  ...
  const result = await collectYield("itdbone", account, (body.to ?? "account").trim());
  if (!result.ok)
    return NextResponse.json({ error: result.error }, { status: result.status });

  void notifyAccount(
    id,
    `🏛 <b>ITDBONE yield collected</b>\n\n` +
